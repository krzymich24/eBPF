...
int xdp_prog1(struct CTXTYPE *ctx) { //eBPF program
    ...
    if (h_proto == htons(ETH_P_IP)) {
        ...
        if (index == 1 ) {
            ...
            if (value_src) {
                bpf_trace_printk("Blocked ICMP packet from source IP: %u, to destination IP: %u, blocked by source IP\n", timestamp, src_ip, dest_ip);
                return XDP_DROP; // Drop the packet from blocked IP address
            }else if (value_dest) {
                bpf_trace_printk("Blocked ICMP packet from source IP: %u, to destination IP: %u, blocked by destination IP\n", timestamp, src_ip, dest_ip);
                return XDP_DROP; // Drop the packet from blocked IP address
            }else{
                bpf_trace_printk("Pass ICMP packet from source IP: %u, to destination IP: %u\n", timestamp, src_ip, dest_ip);
                return XDP_PASS;
            }
            ...
            if(value_src_ip){
                ...
                if (port_value_src_src) {
                    ...
                    bpf_trace_printk("Blocked by source IP: %u, source port: %u\n", timestamp, src_ip, src_port);
                ...
                if (port_value_src_dest) {
                    bpf_trace_printk("Blocked TCP packet from source IP: %u, to destination IP: %u\n", timestamp, src_ip, dest_ip);
                    bpf_trace_printk("Blocked by source IP: %u, destination port: %u\n", timestamp, src_ip, dest_port);
                    return XDP_DROP; // Drop the packet from a blocked IP and port
                }
                ...
                if (port_value_dest_src) {
                    bpf_trace_printk("Blocked TCP packet from source IP: %u, to destination IP: %u\n", timestamp, src_ip, dest_ip);
                    bpf_trace_printk("Blocked by destination IP: %u, source port: %u\n", timestamp, dest_ip, src_port);
                    return XDP_DROP; // Drop the packet from a blocked IP and port
                }
                ...
                if (port_value_dest_dest) {
                    bpf_trace_printk("Blocked TCP packet from source IP: %u, to destination IP: %u\n", timestamp, src_ip, dest_ip);
                    bpf_trace_printk("Blocked by destination IP: %u, destination port: %u\n", timestamp, dest_ip, dest_port);
                    return XDP_DROP; // Drop the packet from a blocked IP and port
                }
                ...
                bpf_trace_printk("Pass TCP packet from source IP: %u, to destination IP: %u\n", timestamp, src_ip, dest_ip);
                bpf_trace_printk("Pass TCP packet from source port: %u, to destination port: %u\n", timestamp, src_port, dest_port);
                return XDP_PASS;
            }
            ...
            if(value_src_ip){
                ...
                bpf_trace_printk("Pass UDP packet from source IP: %u, to destination IP: %u\n", timestamp, src_ip, dest_ip);
                bpf_trace_printk("Pass UDP packet from source port: %u, to destination port: %u\n", timestamp, src_port, dest_port);
                return XDP_PASS;
            }
            
        } else {
            bpf_trace_printk("Pass diffrent packet", timestamp);
            return XDP_PASS;
        }
    }
    return rc;
